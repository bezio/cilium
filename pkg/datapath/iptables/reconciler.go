...
func reconciliationLoop(
	...
	// Only create the timer if periodic reconciliation is enabled (interval > 0).
...
stop:
	for {
		...
		case <-ticker.C():
			...
			// Only reset if periodic reconciliation is enabled (interval > 0).
			if refresher != nil && fullReconciliationInterval > 0 {
				if !refresher.Stop() {
					select {
					case <-ticker.C():
					default:
